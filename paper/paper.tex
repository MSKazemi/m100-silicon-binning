...
per-slice marginals, we show harvested slices cluster ($z=-20.2$), and a fitted pairwise-interaction
model separates that clustering from per-slice propensity, showing both are present and that the two
procurement lots differ in each. Thermal analysis over 240 sockets independently recovers the
sensor-index ordering as the physical arrangement of slices on the die (linear fit $z=-6.13$; an
unconstrained search returns the same ordering reversed, $|\rho|=0.993$), so the clustering is
spatial rather than merely index-space, and confirms that the two cores of a slice are physically
abutted ($r=+0.41$ within-slice versus $+0.32$ across a slice boundary at identical index distance).
