...
with markedly different harvesting statistics. Finally, although POWER9 provides field mechanisms
that could alter the active set after manufacture---predictive deallocation and persistent GARD
records---we find \textbf{no evidence that any of them fired}: 94.4\% of sockets never change
configuration, every observed transition coincides with a reboot rather than occurring during
operation, and apparent sub-16 configurations are collection dropout (99.6\% report a strict subset
of the socket's modal set). Harvest maps behave as static, per-die manufacturing constants.
\end{abstract}
...
\todo{set-stability numbers are over 2020-03..2020-08 and dropout numbers over 2020-03..2020-09;
this inconsistency and the partial coverage both disappear once the 31-month
sweep completes.}
