...
\label{sec:layout}
Free classical MDS on the pooled $24\times24$ matrix is dominated by noise (leading axis 21.3\% of
variance, correlation with core index only $+0.28$), so we instead score explicit candidate
floorplans by correlating predicted physical distance against measured thermal coupling
(Fig.~\ref{fig:layout}). The identity $1\times12$ linear arrangement scores $-0.283$, better than a
random ordering ($z=-2.34$, $p=0.023$) but \emph{worse} than a $3\times4$ grid ($-0.585$); a
hill-climbing search finds an ordering scoring $-0.760$ whose rank correlation with the identity
ordering is $-0.36$.
